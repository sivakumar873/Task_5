...
public class SavingAccount { // Instance Variables
	private String SavingAccountNumber, CustomerName, CustomerID;
	private float Balance = -1, Amount = -1;

	public SavingAccount(String SA_Num, String Cust_Name, String Cust_ID, String Amt) { // Constructor One with three
																						// parameters
	...
	public boolean openAcct() {
		...
		try {
			if (!done) {
				...
				String SQL_Command = "SELECT SavingAccountNumber FROM SavingAccount WHERE SavingAccountNumber ='"
						+ SavingAccountNumber + "'"; // SQL query command
				ResultSet Rslt = Stmt.executeQuery(SQL_Command); // Inquire if the username exsits.
				done = !Rslt.next();
				if (done) {
					SQL_Command = "INSERT INTO SavingAccount(SavingAccountNumber, CustomerName, Balance, CustomerID)" +
							" VALUES ('" + SavingAccountNumber + "','" + CustomerName + "','" + Amount + "', '"
	...
	public boolean deposit(String C_ID) {
		...
		try {
			if (done) {
				...
				String SQL_Command = "SELECT Balance FROM SavingAccount WHERE SavingAccountNumber = '"
						+ SavingAccountNumber + "' AND CustomerID ='" + C_ID + "'";
				...
				SQL_Command = "UPDATE SavingAccount SET Balance = '" + Balance + "' WHERE SavingAccountNumber = '"
						+ SavingAccountNumber + "'";
				System.out.println(SQL_Command);
