	...
	public boolean signUp() {
		...
		try {
			if (done) {
				...
				String SQL_Command = "SELECT Username FROM Account WHERE Username ='" + Username + "'"; // SQL query
																										// command
				ResultSet Rslt = Stmt.executeQuery(SQL_Command); // Inquire if the username exsits.
				done = done && !Rslt.next();
				if (done) {
					SQL_Command = "INSERT INTO Account(Username, Password, Name) VALUES ('" + Username + "','"
							+ Password + "','" + Name + "')"; // Save the username, password and Name
	...
	public String signIn() {
		...
		try {
			if (done) {
				...
				Connection DBConn = ToDB.openConn();
				Statement Stmt = DBConn.createStatement();
				String SQL_Command = "SELECT Name FROM Account WHERE Username ='" + Username + "'AND Password ='"
