...
public class Transaction { // Instance Variables
	private String TransactionNumber, TransactionType, TransactionTime, TransactionDate, FromAccount, ToAccount,
			CustomerID;
	...
	public Transaction(String ToAcc, String FromAcc, String Cust_ID, String Amt, String Trans_type) { // Constructor One
																										// with four
																										// parameters
		ToAccount = ToAcc;
		FromAccount = FromAcc;
		...
		try {
			if (done) {
				...
				while (done) {
					...
					SQL_Command = "SELECT TransactionNumber FROM Transactions WHERE TransactionNumber ='"
							+ TransactionNumber + "'"; // SQL query command
				...
				if (!done) {
					...
					SQL_Command = "INSERT INTO Transactions(TransactionNumber, TransactionType, TransactionAmount, TransactionTime, TransactionDate, FromAccount, ToAccount, CustomerID)"
							+
							" VALUES ('" + TransactionNumber + "','" + TransactionType + "','" + Amount + "','"
							+ TransactionTime + "', '" + TransactionDate + "', '" + FromAccount + "', '" + ToAccount
							+ "', '" + CustomerID + "')"; // Save the username, password and Name
	...
	public Vector searchTransaction(String UName) {
		...
		try {
			...
			Connection DBConn = ToDB.openConn();
			Statement Stmt = DBConn.createStatement();
			String SQL_Command = "SELECT * FROM Transactions WHERE TransactionDate BETWEEN '" + Startdate + "' AND '"
					+ Enddate + "'"; // SQL query command
