...
public class CheckingAccount {
	...
	public boolean openAcct() {
		...
		try {
			if (!done) {
				...
				String SQL_Command = "SELECT CheckingAccountNumber FROM CheckingAccount WHERE CheckingAccountNumber ='"
						+ CheckingAccountNumber + "'"; // SQL query command
				ResultSet Rslt = Stmt.executeQuery(SQL_Command); // Inquire if the username exsits.
				done = !Rslt.next();
				if (done) {
					SQL_Command = "INSERT INTO CheckingAccount(CheckingAccountNumber, CustomerName, Balance, CustomerID)"
							+
							" VALUES ('" + CheckingAccountNumber + "','" + CustomerName + "'," + Amount + ", '"
	...
	public float getBalance() { // Method to return a CheckingAccount balance
		try {
			...
			Connection DBConn = ToDB.openConn();
			Statement Stmt = DBConn.createStatement();
			String SQL_Command = "SELECT Balance FROM CheckingAccount WHERE CheckingAccountNumber ='"
					+ CheckingAccountNumber + "'"; // SQL query command for Balance
			ResultSet Rslt = Stmt.executeQuery(SQL_Command);
	...
	public boolean deposit(String C_ID) {
		...
		try {
			if (done) {
				...
				String SQL_Command = "SELECT Balance FROM CheckingAccount WHERE CheckingAccountNumber = '"
						+ CheckingAccountNumber + "' AND CustomerID ='" + C_ID + "'";
				...
				SQL_Command = "UPDATE CheckingAccount SET Balance = '" + Balance + "' WHERE CheckingAccountNumber = '"
